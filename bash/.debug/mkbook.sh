...
bookcreate() {
	if [ ! -d $name ]; then
		...
		cp $HOME/scripts/latex/template/base/base.tex $(pwd)/$name/$name.tex
		cp $HOME/scripts/latex/template/base/newchapter.tex $(pwd)/$name/chapters/newchapter.tex
...
maketitle () {
	...
	if [ -n $lang ]; then
		if [ $lang == "en" ]; then
			cp $HOME/scripts/latex/template/title/en/title.tex $(pwd)/$name/title/title.tex
			sed -i 's/..\/<++>/..\/'$name'.tex/g' $(pwd)/$name/title/title.tex
		elif [ $lang == "it" ]; then
			cp $HOME/scripts/latex/template/title/it/title.tex $(pwd)/$name/title/title.tex
			sed -i 's/..\/<++>/..\/'$name'.tex/g' $(pwd)/$name/title/title.tex
		else
...
titlecolor() {
	if [ $opt == 1 ]; then
		...
		cp $HOME/scripts/latex/template/base/sapienza.tex $(pwd)/$name/$name.tex
	fi
}
helpf() {
